...
function makeAcpAgentWrapper(dir: string, env: Record<string, string>): string {
  ...
  const agentPath = NodePath.join(binDir, "kimi");
...
function withFakeAcpAgent<A, E, R>(
  ...
  return Effect.gen(function* () {
    ...
    const config = decodeKimiSettings({ binaryPath: agentPath });
...
it.layer(KimiTextGenerationTestLayer)("KimiTextGeneration", (it) => {
  it.effect(
    "advertises terminal capability and sets the model via session/set_config_option",
    () => {
      ...
      return withFakeAcpAgent(
        {
          ...
          T3_ACP_PROMPT_RESPONSE_TEXT: JSON.stringify({
            ...
            body: "- verify kimi acp model config path",
          }),
        },
        (textGeneration) =>
          Effect.gen(function* () {
            const generated = yield* textGeneration.generateCommitMessage({
              cwd: process.cwd(),
              branch: "feature/kimi-text-generation",
              stagedSummary: "M apps/server/src/textGeneration/KimiTextGeneration.ts",
              stagedPatch:
                "diff --git a/apps/server/src/textGeneration/KimiTextGeneration.ts b/apps/server/src/textGeneration/KimiTextGeneration.ts",
              modelSelection: {
            ...
            expect(generated.body).toBe("- verify kimi acp model config path");
    ...
    withFakeAcpAgent(
      ...
      (textGeneration) =>
        Effect.gen(function* () {
          const generated = yield* textGeneration.generateCommitMessage({
            cwd: process.cwd(),
            branch: "feature/kimi-noisy-json",
  ...
  it.effect("generates thread titles through Kimi ACP text generation", () =>
    withFakeAcpAgent(
      {
        T3_ACP_PROMPT_RESPONSE_TEXT: JSON.stringify({
  ...
  it.effect("closes the ACP child process after text generation completes", () => {
    const exitLogDir = NodeFS.mkdtempSync(
      NodePath.join(NodeOS.tmpdir(), "t3code-kimi-text-exit-log-"),
    );
    ...
    return withFakeAcpAgent(
      ...
      (textGeneration) =>
        Effect.gen(function* () {
          const generated = yield* textGeneration.generateCommitMessage({
            cwd: process.cwd(),
            branch: "feature/kimi-runtime-close",
            stagedSummary: "M apps/server/src/textGeneration/KimiTextGeneration.ts",
            stagedPatch:
              "diff --git a/apps/server/src/textGeneration/KimiTextGeneration.ts b/apps/server/src/textGeneration/KimiTextGeneration.ts",
            modelSelection: {
