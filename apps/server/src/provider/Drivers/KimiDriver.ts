/**
 * KimiDriver — `ProviderDriver` for the Kimi CLI (`kimi`) runtime.
 *
 * Kimi exposes a standard ACP server over stdio via `kimi acp`. Model
 * catalog and capability refreshes happen during the managed provider
 * status check by reading the session `configOptions` (the `model` and
 * `thought_level` selects) from a short-lived probe session.
 *
