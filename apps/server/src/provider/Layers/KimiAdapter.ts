...
import { kimiModeStateFromConfigOptions, resolveKimiAcpBaseModelId } from "./KimiProvider.ts";
...
// Kimi's unrestricted modes. Checked before the generic implement aliases in
// full-access so T3 full-access maps to `yolo`/`auto` rather than `default`;
// approval-required skips this list and lands on `default` instead.
const ACP_FULL_ACCESS_MODE_ALIASES = ["yolo", "auto"];
...
function applyRequestedSessionConfiguration<E>(input: {
  ...
  return Effect.gen(function* () {
    ...
    const requestedModeId = resolveRequestedModeId({
      ...
      modeState:
        (yield* input.runtime.getModeState) ??
        kimiModeStateFromConfigOptions(yield* input.runtime.getConfigOptions),
    });
    if (!requestedModeId) {
      return;
    }
...
export function makeKimiAdapter(kimiSettings: KimiSettings, options?: KimiAdapterLiveOptions) {
  return Effect.gen(function* () {
    ...
    const mapAcpFailure = <A, E, R>(effect: Effect.Effect<A, E, R>) =>
      effect.pipe(
        Effect.mapError(
          (cause) =>
            new EffectAcpErrors.AcpTransportError({
              detail: "Failed to process Kimi ACP event.",
              cause,
            }),
        ),
      );
